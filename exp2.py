...
# Experiment 2, bootstraping with a lot of nodes and passive listening for infohashes
...
class KRPCRequestHandler(SocketServer.BaseRequestHandler):
    def handle(self):
        ...
        if rpc_message_type == "r":
            if rpc_message["r"].has_key("nodes"):
                self.server.dht_node.discovered_nodes_count += 1
    ...
    def _handle_get_peers(self, rpc_message):
        if rpc_message["a"]["info_hash"] not in self.server.dht_node.discovered_info_hashes:
            print "Discovered info hash: ", rpc_message["a"]["info_hash"]
            self.server.dht_node.discovered_info_hashes.append(rpc_message["a"]["info_hash"])
        if len(self.server.dht_node.discovered_info_hashes) == 10:
            print "Discovered 10 infohashes :", self.server.dht_node.discovered_info_hashes
            print "Number of nodes discovered: ", self.server.dht_node.discovered_nodes_count
            return
        response = bencode.encode({"t": rpc_message["t"],
                                   "y": "r",
                                   "r": {
